...
function MenuEditor(idSelector, options) {
    ...
    var settings = {
        ...
        iconPicker: { cols: 6, rows: 6, footer: false, iconset: "fontawesome5" },
        maxLevel: -1,
        listOptions: { 
    ...
    function TButtonGroup() {
        ...
        var $btnEdit = TButton({classCss: 'btn btn-primary btn-sm btnEditNew', text: settings.labelEdit});
        var $btnRemv = TButton({classCss: 'btn btn-danger btn-sm btnRemoveNew', text: settings.labelRemove});
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var $li = $('<li>').addClass('list-group-item pr-0').attr("data-id",v.id);
            $li.data(itemObject);
